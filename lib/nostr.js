...
export { fetchEventsManaged } from './connection-manager'
export {
  storePrivateKey,
  hasPrivateKey,
  getPrivateKeyHex,
...
import { getPendingPublishOutbox, getPublishOutboxStats } from './publish-outbox'
...
/**
 * Check whether an encrypted persistent private key exists for the current user.
 * This does not decrypt or load the key into memory.
 * @param {string} [pubkey]
 * @returns {boolean}
 */
export function hasPersistedStoredPrivateKey(pubkey) {
  return KeyStore.hasPersistedPrivateKey(pubkey)
}

/**
 * Restore a previously exported private key into memory without prompting.
 * @param {string} pubkey
 * @returns {Promise<boolean>}
 */
export async function restoreStoredPrivateKey(pubkey) {
  return KeyStore.restorePrivateKey(pubkey)
}
...
/**
 * Publish event to relays with structured delivery information.
 * If the event is unsigned, it is signed using the active signer first.
 */
export async function publishEventResult(event, relays = [getDefaultRelay()], options = {}) {
  const validRelays = filterValidRelays(relays)
  try {
    const signedEvent = event?.id && event?.sig ? event : await signEventNip07(event)
    return await publishManagedResult(signedEvent, validRelays, options)
  } catch (e) {
    ...
    return {
      ...
      ok: false,
      okRelays: [],
      failedRelays: validRelays,
      firstOkMs: 0,
      retryQueued: !!event?.id,
 ...
 *
 * @param {Object} event - The signed or unsigned Nostr event to publish
 * @param {string[]} [relays] - Array of relay URLs
 * @returns {Promise<boolean>} True if published successfully
 */
export async function publishEvent(event, relays = [getDefaultRelay()]) {
  const result = await publishEventResult(event, relays)
  return !!result.ok
}

export async function retryPendingPublishOutbox(limit = 20) {
  const items = getPendingPublishOutbox(limit)
  const results = []
  for (const item of items) {
    try {
      const event = JSON.parse(item.eventJson)
      const relays = item.relayUrls?.length ? item.relayUrls : [getDefaultRelay()]
      results.push(await publishManagedResult(event, filterValidRelays(relays), { enqueueOutbox: false }))
    } catch (e) {
      results.push({ eventId: item.eventId, ok: false, okRelays: [], failedRelays: item.relayUrls || [], firstOkMs: 0, retryQueued: true, error: e?.message || 'retry failed' })
    }
  }
  return results
}

export { getPendingPublishOutbox, getPublishOutboxStats }
...
export async function searchNotes(query, options = {}) {
  ...
  try {
    const filter = {
      kinds: [1, NOSTR_KINDS.ADDRESSABLE_SHORT_VIDEO],
      search: query,
      limit
    }
...
/**
 * Get notification (inbox/read) relays for fetching events targeting the user.
 * Mirrors Android NostrRepositoryNotifications.fetchNotifications:
 *   1) localStorage `nip65Relays` で read=true のもの (RelaySettings ミニアプリで保存される)
 *   2) NIP-65 (kind 10002) のリモート read relays
 *   3) フォールバックでデフォルトリレー
 * @param {string} [pubkey] - User's pubkey (uses stored if not provided)
 * @returns {Promise<string[]>} Array of read relay URLs
 */
export async function getNotificationRelays(pubkey) {
  // 1) Local NIP-65 list (RelaySettings ミニアプリで管理) — Android prefs.nip65Relays.filter { it.read } 相当
  if (typeof window !== 'undefined') {
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed)) {
          const localReads = parsed
            .map(r => {
              if (typeof r === 'string') return { url: r, read: true }
              return r
            })
            .filter(r => r && r.url && r.read !== false)
            .map(r => (r.url || '').trim().replace(/\/+$/, ''))
            .filter(u => isValidRelayUrl(u))
            .filter((u, i, arr) => arr.indexOf(u) === i)
          if (localReads.length > 0) return localReads
        }
      }
    } catch (e) {
      console.warn('[nostr] getNotificationRelays: failed to parse local nip65Relays:', e?.message || e)
    }
  }
  ...
  if (userPubkey) {
    try {
      const relayList = await fetchRelayListMetadata(userPubkey)
      if (relayList.read.length > 0) return relayList.read
    } catch (e) {
      console.error('[nostr] getNotificationRelays: failed to fetch NIP-65:', e?.message || e)
    }
  }
...
// ============================================
// NIP-51 Bookmarks (Kind 10003) + NIP-57 Zap Totals + Repost Counts
//   iOS NostrRepository+Bookmarks.swift / +Timeline.swift と同じ振る舞いに揃える。
//   PostActions（タイムライン／ホーム）から呼ぶ集約 API。
// ============================================

/**
 * Fetch the user's latest Kind 10003 bookmark list event IDs.
 * Returns an array of event ids that are bookmarked (\"e\" tags only).
 * @param {string} pubkey - hex pubkey of the user whose bookmarks to fetch
 * @returns {Promise<string[]>}
 */
export async function fetchBookmarkEventIds(pubkey) {
  if (!pubkey) return []
  try {
    const events = await fetchEvents(
      { kinds: [NOSTR_KINDS.BOOKMARKS], authors: [pubkey], limit: 5 },
      getReadRelays()
    )
    if (!events || events.length === 0) return []
    // Take latest by created_at (replaceable event)
    const latest = events.sort((a, b) => b.created_at - a.created_at)[0]
    const ids = []
    for (const t of latest.tags || []) {
      if (t[0] === 'e' && t[1]) ids.push(t[1])
    }
    return ids
  } catch (e) {
    console.error('Failed to fetch bookmark list:', e)
    return []
  }
}

/**
 * Add an event id to the user's NIP-51 bookmark list (Kind 10003).
 * Idempotent — existing entries are preserved, duplicates are skipped.
 * @param {string} pubkey - user hex pubkey
 * @param {string} eventId - event id to bookmark
 * @returns {Promise<{success: boolean, ids: string[]}>}
 */
export async function addBookmark(pubkey, eventId) {
  if (!canSign()) throw new Error('署名機能が必要です')
  if (!pubkey || !eventId) throw new Error('invalid arguments')

  const existing = await fetchBookmarkEventIds(pubkey)
  if (existing.includes(eventId)) {
    return { success: true, ids: existing }
  }
  const next = [...existing, eventId]
  const tags = next.map(id => ['e', id])
  const tmpl = createEventTemplate(NOSTR_KINDS.BOOKMARKS, '', tags)
  tmpl.pubkey = pubkey
  const signed = await signEventNip07(tmpl)
  const success = await publishEvent(signed)
  if (!success) throw new Error('ブックマークの公開に失敗しました')
  return { success: true, ids: next }
}

/**
 * Remove an event id from the user's NIP-51 bookmark list (Kind 10003).
 * Idempotent — if the event isn't bookmarked, this is a no-op.
 * @param {string} pubkey - user hex pubkey
 * @param {string} eventId - event id to un-bookmark
 * @returns {Promise<{success: boolean, ids: string[]}>}
 */
export async function removeBookmark(pubkey, eventId) {
  if (!canSign()) throw new Error('署名機能が必要です')
  if (!pubkey || !eventId) throw new Error('invalid arguments')
...
/**
 * Fetch repost counts (Kind 6) for the given target event ids.
 * @param {string[]} eventIds
 * @returns {Promise<{counts: Record<string, number>, myRepostIds: Record<string, string>}>}
 *   counts:        eventId -> repost count
 *   myRepostIds:   eventId -> the current user's repost event id (for undo)
 */
export async function fetchRepostCounts(eventIds, myPubkey) {
  if (!eventIds || eventIds.length === 0) {
    return { counts: {}, myRepostIds: {} }
  }
  try {
    const events = await fetchEvents(
    ...
    for (const ev of events) {
      ...
      if (myPubkey && ev.pubkey === myPubkey) {
        myRepostIds[targetId] = ev.id
      }
    }
    return { counts, myRepostIds }
  } catch (e) {
    console.error('Failed to fetch repost counts:', e)
    return { counts: {}, myRepostIds: {} }
  }
}

/**
 * Fetch zap totals (Kind 9735) for the given target event ids, summed as sats.
 * @param {string[]} eventIds
 * @returns {Promise<Record<string, number>>} eventId -> total sats
 */
export async function fetchZapTotals(eventIds) {
  if (!eventIds || eventIds.length === 0) return {}
  try {
    const events = await fetchEvents(
    ...
    const totals = {}
    for (const ev of events) {
      const parsed = parseZap(ev)
      if (!parsed || !parsed.targetEventId || !parsed.amount) continue
      totals[parsed.targetEventId] = (totals[parsed.targetEventId] || 0) + parsed.amount
    }
    return totals
  } catch (e) {
    console.error('Failed to fetch zap totals:', e)
    return {}
  }
}
