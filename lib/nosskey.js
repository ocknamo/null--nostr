...
// 一部のパスワードマネージャー (Bitwarden 等) はパスキーの PRF 拡張に未対応で、
// パスキー自体は選べても鍵導出 (認証) に失敗する。失敗時の案内文言。
const PASSKEY_PM_HINT =
  'Bitwardenなど一部のパスワードマネージャーはパスキー(PRF)に対応していない場合があります。'

/**
 * Map a passkey/WebAuthn error to a concise, user-facing message.
 * User cancellation returns the given `cancelled` text; other failures return
 * a generic message plus a hint that some password managers don't support the
 * passkey PRF extension.
 * @param {unknown} error
 * @param {{ cancelled?: string }} [options]
 * @returns {string}
 */
export function passkeyErrorMessage(error, { cancelled = 'パスキー認証がキャンセルされました' } = {}) {
  ...
  if (msg.includes('No credentials')) {
    return 'パスキーが見つかりません。新規登録してください。'
  }
  return `パスキーの認証に失敗しました。${PASSKEY_PM_HINT}`
}
